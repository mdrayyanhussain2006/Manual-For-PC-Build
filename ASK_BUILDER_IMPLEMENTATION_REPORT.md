...
| `src/lib/ask-builder/providers/DeepSeekProvider.ts` | DeepSeek V4 implementation |
| `src/lib/ask-builder/providers/MockAIProvider.ts` | Deterministic mock for development |
| `src/lib/ask-builder/knowledge.ts` | System prompt builder (ComponentRegistry + context) |
| `src/lib/ask-builder/actionValidator.ts` | Action security validation layer |
| `src/lib/ask-builder/contextBridge.ts` | IntegrationState → AskBuilderContext snapshot |
| `src/pages/api/ask-builder.ts` | Server-side API endpoint |
...
| `output/ASK_BUILDER_EVIDENCE/` | 27 browser screenshots |
...
| `src/styles/global.css` | Appended 400+ lines of `.ab-*` scoped styles |
...
When `AI_API_KEY` is absent, the server automatically uses `MockAIProvider`. This means:
...
| Clear works | ✅ |
| Mock provider works | ✅ |
| DeepSeek provider boundary exists | ✅ |
...
| Security scan clean | ✅ |
| Build passes | ✅ (exit 0) |
| Browser verification passes | ✅ (27 screenshots) |
...
2. Set `AI_API_KEY=sk-...` (your DeepSeek API key)
3. Optionally set `AI_MODEL=deepseek-chat` (or another model)
4. Restart the dev server or redeploy
5. Ask Builder will automatically switch from Mock to DeepSeek

**Never commit `.env` to version control.**

For production hosting (Vercel, Netlify, Railway, etc.), add `AI_API_KEY` as a server-side environment variable in the hosting dashboard.
